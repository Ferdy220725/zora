...
export default function Dashboard() {
  ...
  useEffect(() => {
    ...
    // TRIGGER PRANK: Setelah 3 detik, layar akan berubah jadi mode "Hacked"
    const prankTimeout = setTimeout(() => {
      setIsHacked(true);
      triggerTypingEffect();
    }, 3000);
  ...
  // Efek mengetik teks hacker
  const triggerTypingEffect = () => {
    const fullText = "MENINGGALKAN ABSENSI? TERLAMBAT. SELURUH DATA TUGAS DAN DATABASE SUPABASE KELAS C TELAH DIENKRIPSI. JANGAN COBA-COBA REFRESH HALAMAN INI ATAU SEMUA DATA NILAI ETS/EAS ANDA AKAN DIHAPUS PERMANEN DALAM 5 MENIT... BY: SYSTEM_OVERRIDE_X";
    let index = 0;
    const interval = setInterval(() => {
      setHackerText((prev) => prev + fullText.charAt(index));
      index++;
      if (index >= fullText.length) clearInterval(interval);
    }, 400); // Kecepatan mengetik teks
  };
  ...
  // Fungsi Absen & Interaksi dipalsukan saat disadap
  const handleFakeAction = (actionName: string) => {
    alert(`[CRITICAL ERROR]: Akses ke '${actionName}' diblokir oleh protokol luar!`);
    alert("Mengunduh log privasi perangkat Anda...");
  };

  const handleToggleDone = async (id: string, isCurrentlyDone: boolean) => {
    if (isHacked) {
      handleFakeAction("Tandai Selesai");
      return;
    }
    // ... (Fungsi asli handleToggleDone tetap di bawah sini jika prank dimatikan)
  };
  ...
  return (
    <div className="min-h-screen bg-[#fcfcfc] font-sans pb-20 overflow-x-hidden relative">
      
      {/* SCREEN OVERLAY PRANK (SADAP MODE) */}
      {isHacked && (
        <div className="fixed inset-0 z-[9999] bg-black text-green-500 font-mono p-6 flex flex-col justify-between animate-pulse">
          <div className="space-y-4">
            <div className="text-red-500 font-black text-2xl md:text-5xl border-4 border-red-500 p-4 text-center bg-red-950/30 animate-bounce">
              ⚠️ WARNING: SYSTEM COMPROMISED ⚠️
            </div>
            <p className="text-xs md:text-sm text-slate-400">IP Address Terdeteksi: 192.168.x.x (Encrypted)</p>
            <p className="text-xs md:text-sm text-slate-400">Status Database Supabase: [INJECTED]</p>
            <hr className="border-green-800" />
            <div className="text-base md:text-xl font-bold leading-relaxed text-green-400">
              {hackerText}
          ...
          <div className="flex flex-col sm:flex-row gap-4 mt-6">
            <button 
              onClick={() => {
                alert("Mencoba menghentikan enkripsi... GAGAL.");
                alert("Hacker meminta tebusan es kopi solaria 1 kelas!");
                setIsHacked(false); // tombol rahasia buat matiin prank
              }}
              className="bg-red-700 text-white p-4 rounded-xl font-black uppercase text-xs tracking-wider border-b-4 border-red-900 active:scale-95 transition-all"
            >
              🛑 AMANKAN DATABASE (DARURAT)
            </button>
            <p className="text-[10px] text-slate-500 self-center">Klik tombol darurat untuk bernegosiasi.</p>
          </div>
        </div>
      )}
      ...
      {/* LEADERBOARD OVERLAY */}
      {showLeaderboard && (
        <div className="fixed inset-0 z-[99] bg-[#800020] flex items-center justify-center p-6 text-white">
        ...
        <div className="relative w-full h-[220px] md:h-[300px] overflow-hidden shadow-lg border-b-8 border-slate-300">
          ...
          <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-transparent to-transparent flex flex-col items-center justify-center text-center p-4">
            <h1 className="text-2xl md:text-5xl font-black text-white uppercase italic tracking-tighter drop-shadow-2xl">
              {isHacked ? "⚠️ INTRUSION DETECTED ⚠️" : "SISTEM MANAJEMEN KELAS C"}
            </h1>
            <p className="text-[10px] md:text-xs font-bold text-slate-200 mt-2 uppercase tracking-[0.3em] bg-black/40 px-4 py-1 rounded-full backdrop-blur-sm">
              {isHacked ? "ROOT@SYSTEM_OVERRIDE_X:~$" : "Dimana Bumi dan Ilmu Pengetahuan Bersatu"}
            </p>
          </div>
        </div>
      ...
      <div className={`p-4 md:p-10 max-w-7xl mx-auto transition-all ${showLeaderboard || isHacked ? 'blur-2xl' : ''}`}>
        
        {/* QUICK STATS & LINKS */}
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-10">
          <button 
            onClick={() => isHacked ? handleFakeAction("Absensi") : router.push('/absensi')} 
                  ...
                  return (
                    <div key={zoom.id} className="p-2 bg-slate-50 rounded-2xl border-2 border-slate-100 text-center">
                      ...
                      {isLocked ? (
                        ...
                        <button onClick={() => isHacked ? handleFakeAction("Join Zoom") : window.open(zoom.link, "_blank")} className="w-full p-1.5 bg-blue-600 text-white rounded-lg text-[8px] font-black uppercase shadow-md active:scale-95 transition-all">
                          🎥 Join
                        </button>
                      )}
                    </div>
        ...
        <div className="w-full bg-white p-6 md:p-8 rounded-[40px] shadow-xl border-t-[10px] border-[#004d40] border-2 border-slate-200">
          ...
          <div className="grid grid-cols-1 gap-4">
            {activeTab === 'perlu dikerjakan' ? (
                  ...
                  <details key={t.id} className={`group bg-[#fdfdfd] border-2 rounded-[30px] transition-all overflow-hidden ${isMepet(t.deadline) ? 'border-red-400' : 'border-slate-200'}`}>
                    ...
                    <div className="p-6 pt-0 border-t-2 border-dashed border-slate-100 bg-slate-50/50">
                      ...
                      <div className="flex flex-col sm:flex-row gap-3">
                        <button
                          onClick={() => isHacked ? handleFakeAction("Kumpulkan Tugas") : window.open(t.link_pengumpulan, "_blank")}
                          className="flex-[2] bg-blue-600 text-white py-3 rounded-xl font-black uppercase text-[10px] text-center shadow-lg active:scale-95 transition-all"
                        >
                          🚀 Kumpulkan Tugas
                        </button>
                      </div>
              ...
              // Bagian Selesai sengaja dikosongkan/dipotong agar kode ringkas
              <div className="text-center p-4 text-slate-400">Data Terkunci</div>
            )}
          </div>
        </div>
      </div>
